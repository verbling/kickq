 ...
 * @private
 */
Process.prototype._workFinish = function( job, optErr, optDone ) {
  if (this._disposed) {
    return;
  }
  var done = noop;
  ...
  processedPromise.ensure( jobModel.dispose.bind(jobModel) );
  processedPromise.ensure( done );
  processedPromise.ensure( this._masterLoop.bind(this) );
};
...
Process.prototype._workTimeout = function( job ) {
  ...
  prom.ensure(jobModel.dispose.bind(jobModel));
  prom.ensure( this._masterLoop.bind(this) );
};
