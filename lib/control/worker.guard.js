 ...
 * up the internal scheduler if run for first time.
 *
 * @param {!kickq.Worker} worker The Process ctrl instance.
 * @constructor
 */
...
Guard.prototype._onError = function(err) {
  if (!err.message.match(/ECONNREFUSED/)) {
    log.warn('_onError() :: Non Redis error: ', err.message);
    return;
  }
  ...
  log.error('_onError() :: Redis Down! Waiting for reconnect. Error: ', err.message);
...
Guard.prototype._checkProcessingWorkers = function() {
  ...
  this.worker.processing.forEach(function(processItem, jobId) {
