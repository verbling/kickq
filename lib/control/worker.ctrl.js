...
Worker.prototype._workStart = function( job ) {
  job._processCount = ++processCount;
  log.info('_workStart() :: Init. job.id, job.name, _disposed, _processCount: ',
    job.id, job.name, this._disposed, job._processCount );
  ...
  try {
    consumerReturn = this.consumerWorkerFn.call(
      ...
      job.getPublic(),
      job.data,
      this._workFinish.bind(this, job, null)
    );
  } catch (ex) {
    log.fine('_workStart() :: Consumer callback failed. ex: ', ex);
    this._workFinish(job, false, ex);
    throw ex;
  }
  ...
  if (when.isPromise(consumerReturn)) {
    consumerReturn.then(
      this._workFinish.bind(this, job, true),
      this._workFinish.bind(this, job, false)
    );
  }
};
 ...
 * @param {boolean} If the job finished successfully.
...
Worker.prototype._workFinish = function( job, success, optErr, optDone ) {
  ...
  processedPromise.always( log.fine.bind(log, '_workFinish() :: Model finished' +
    ' process operation.'));
...
Worker.prototype.processTimeout = function( job ) {
  log.fine('processTimeout() :: Init. jobId: ', job.id, ' Queue:', job.name +
    ' processTimeout:', job.processTimeout, ' processCount:', job._processCount);
