...
Worker.prototype.startWorker = function() {
  ...
  if (!redisCtrl.connected) {
    // wait for manager to wake us up
    return;
  }
...
Worker.prototype._onPopFetchFail = function(err) {
  log.warn('_onPopFetchFail() :: PopModel fetch() method failed, workerId:',
    this.workerId, 'processCount:', this.processCount, 'Error:', err);
 ...
 *   operations are finished.
 * @param {Boolean=} optTimeout Set to true if worker timed out.
 * @private
 */
Worker.prototype._saveWorker = function(status, cb, optTimeout) {
  ...
  jobModel.processed(status, !!optTimeout)
    .bind(this)
    .finally(function() {
      this.savingJobItem = false;
      log.finest('_saveWorker() :: Model finished process operation. Worker Id:',
        this.workerId, 'jobId:', (this.jobItem && this.jobItem.id),
        'ProcessCount:', this.processCount, 'Process-State:',
        (this.processItem && this.processItem.state));
 ...
 * @param {Number} processCount The processCount this callback belongs to.
 */
Worker.prototype.processTimeout = function(processCount) {
  ...
  this._saveWorker(false, noop, true);
};
