 ...
 * @constructor
 */
var Worker = module.exports = function(workerManager) {
...
Worker.prototype._onPopFetchFail = function(err) {
  log.warn('_onPopFetchFail() :: PopModel fetch() method failed:', err);
...
Worker.prototype._workStart = Promise.method(function( jobItem ) {
  ...
  log.info('_workStart() :: Init. job.id:', jobItem.id, 'jobItem.name:', jobItem.name,
    '_disposed:', this._disposed, '_processCount:', jobItem._processCount);
  ...
  try {
    ...
    log.fine('_workStart() :: Consumer callback failed. ex: ', ex);
...
Worker.prototype._workFinish = function(optErr, optDone) {
  ...
  log.fine('_workFinish() :: Init. jobId: ', this.jobItem.id, ' Queue:',
    this.jobItem.name, 'Error: ', optErr, ' processCount:',
    this.jobItem._processCount);
  ...
  jobModel.processed(success)
    .bind(this)
    .finally(function() {
      log.fine('_workFinish() :: Model finished process operation.');
      jobModel.dispose();
...
Worker.prototype.processTimeout = function() {
  log.fine('processTimeout() :: Init. jobId:', this.jobItem.id, 'Queue:',
    this.jobItem.name, 'processTimeout:', this.jobItem.processTimeout,
...
Worker.prototype._onWorkerFail = function(err) {
  log.fine('_onWorkerFail() :: Consumer callback failed. err: ', err);
  this.state = Worker.State.POST_WORKER_ERROR;
