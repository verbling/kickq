...
Worker.prototype.masterLoop = function(optErr) {
  ...
  for (; processingCount < this.concurrentJobs; processingCount++) {
    this.popModel.fetch(this.jobNames)
      .bind(this)
      .catch(this.masterLoop)
      .then(this._workStart)
...
Worker.prototype._workStart = function( job ) {
  ...
  log.info('_workStart() :: Init. job.id:', job.id, 'job.name:', job.name,
    '_disposed:', this._disposed, '_processCount:', job._processCount);
  ...
  if (this.consumerWorkerFn.length === 3) {
    if (consumerReturn && typeof consumerReturn.then === 'function') {
      Promise.try(consumerReturn)
        .bind(this)
        .return(null)
        .then(this._workFinish.bind(this, job))
        .catch(this._workFinish.bind(this, job));
    }
  }
};
...
Worker.prototype._workFinish = function(job, optErr, optDone) {
  ...
  if (success) {
    ...
    processItem.errorMessage = job.lastError = error;
  }
