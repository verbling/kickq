...
JobModel.prototype.save = function() {
  ...
  console.log('JobModel.save() :: init');
...
JobModel.prototype._update = function() {
  ...
  console.log('JobModel.update() :: init. ', key);
  ...
  this.client.hmset( key, redisJobItem, function(err, res) {
    console.log('JobModel._update() :: DB response:', res);

    if (err) {
      console.log('JobModel._update() :: DB FAIL:', err, redisJobItem);
...
JobModel.prototype._finishJob = function(outcome) {
  ...
  var state = (outcome ? states.Job.SUCCESS : states.Job.ERROR);
