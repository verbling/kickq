...
var redisCtrl = require('../control/redis.ctrl').getInstance();
...
Scheduler.prototype._ping = function() {
  if (!redisCtrl.connected) {
    this._noRedisConnection();
    return;
  }

  log.finest('_ping() :: Perfoming atomic zpop on all scheduled queues.',
    'Total Queues:', this.queues.length);
  ...
  Promise.resolve(this.queues)
    ...
    .map(function(queue){
      ...
      return this._zpop(key)
        ...
        .catch(this._onZpopError.bind(this, queue));
    })
    .finally(this._pong);
};
 ...
 * @private
 */
Scheduler.prototype._onZpopError = function(queue, err) {
  log.error('_onZpopError() :: Queue: ', queue, 'Redis-Connected:',
    redisCtrl.connected, 'Error:', err);
};

/**
 * Handles lost connection with Redis.
 *
 */
Scheduler.prototype._noRedisConnection = function() {
  ...
  log.fine('_noRedisConnection() :: Redis connection down, waiting to reconnect...');
