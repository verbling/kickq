...
var Scheduler = module.exports = function() {
  this._count = ++instanceCount;

  log.finer('Ctor() :: Init. instanceCount: ', this._count);
...
Scheduler.prototype._ping = function() {
  log.finest('_ping() :: Init. instanceCount: ', this._count);
  clearTimeout(this._pongTimeout);
  this._pongTimeout = null;
...
Scheduler.prototype._pong = function() {
  ...
  log.finest('_pong() :: Init. Next ping in: ', totalTimeout, ' instanceCount: ', this._count);
...
Scheduler.prototype._onZpopResponse = function(queue, jobIds) {
  ...
  log.fine('_onZpopResponse() :: Init. queue, jobIds: ', queue, jobIds);
  ...
  when.all(promises).then(def.resolve, function(err) {
...
Scheduler.prototype._lineupForQueue = function(queue, jobItem) {
  log.finest('_lineupForQueue() :: Init. queue, jobId:', queue, jobItem.id);
  // defense
