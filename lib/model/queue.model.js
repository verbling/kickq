...
QueueModel.prototype.save = function() {
  ...
  case states.Job.NEW:
    perf.log('QueueModel.save - start');
    // new jobs get added to process queue and check for hotjob option.
    this.addToProcessQueue()
      .then(perf.log.bind(perf,'QueueModel.save - addToProcessQueue() finished'), def.reject)
      .then(this.checkHotjob.bind(this), def.reject)
      .then(perf.log.bind(perf,'QueueModel.save - hotjob finished'), def.reject)
...
QueueModel.prototype.checkHotjob = function() {
  perf.log('QueueModel.checkHotjob - hotjob Start');
  var hotjob = new Hotjob(this.job);
  var promise = hotjob.check();
  perf.log('QueueModel.checkHotjob - hotjob Finish');
  return promise;
};
...
QueueModel.prototype._scheduleRedis = function(queue, delay) {
  ...
  this.job.save().ensure(function(){
    this.client.zadd(this.NS + ':' + queue, scheduleFor, this.job.id, function(err) {
      if (err) {
