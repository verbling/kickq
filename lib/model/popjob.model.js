...
PopModel.prototype.fetch = function(jobNames) {
  log.info('fetch() :: Init. (BLPOP) -- jobNames, instanceCount :: ',
    util.inspect(jobNames), this._instanceCount);
...
PopModel.prototype._onResponse = function(resolver, err, response) {
  log.finer('_onResponse() :: Init. err, instanceCount: ', err, this._instanceCount);
  if (this._disposed) {
    return;
  }
  if (err) {
...
PopModel.prototype._onJobItemFetch = function(resolver, jobItem) {
  log.fine('_onJobItemFetch() :: Init. jobId, state, Queue, instanceCount: ',
    jobItem.id, jobItem.state, jobItem.name, this._instanceCount);
