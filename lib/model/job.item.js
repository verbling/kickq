...
JobItem.ProcessItem = function(job) {
  ...
  /** @type {?Error} if failed the consumer provided error message */
  this.error = null;
...
JobItem.prototype.delete = Promise.method(function() {
  ...
  return new Promise(function(resolve, reject) {
    self.client.multi()
      .del( self._getKey())
      .zrem( self.NS + ':time-index', self.id)
      .srem( states.getKey(self.state), self.id)
      .exec(function(err) {
        // publish anyway
        var pubItem = {
          id: jobId,
...
JobItem.prototype.setState = Promise.method(function(newState) {
  ...
  log.info('setState() :: jobId:', this.id, 'oldState:', this.state,
    'newState:', newState, 'Queue:', this.name);
      ...
      .exec(function(err, response){
        ...
        if (response[1] === 0) {
          log.warn('setState() :: State Set Move failed. Will attempt to create' +
            ' a new record. jobId:', self.id, 'oldState:',  self.state,
            'newState:', newState, 'Queue:', self.name);
