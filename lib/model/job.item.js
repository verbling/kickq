...
JobItem.prototype.save = function() {
  log.fine('save() :: Init. jobId, state, Queue:', this.id, this.state, this.name);
  var def = when.defer();
...
JobItem.prototype.delete = function() {
  ...
  var serialized = this.getPublicJSON();
  var jobId = this.jd;
  var queue = this.name;
  ...
  this.client.multi()
    ...
    .exec(function(err) {
      ...
      var pubItem = {
        id: jobId,
        queue: queue,
        serializedJobItem: serialized,
        status: !!err
      };
      this.client.publish( channels.getKey(channels.Channels.DELETE), pubItem );
      // reset vars
      serialized = jobId = queue = null;
    ...
    .exec(function(err, response){
      ...
      if (0 === response[1]) {
        log.warn('setState() :: State Set Move failed. Will attempt to create' +
          ...
          'newState, Queue :: ', this.id, this.state, newState, this.name);

        this.createState(this.state).then(def.resolve, def.reject);
      } else {
        def.resolve();
      }
    }.bind(this));
 ...
 * @return {when.Promise} a promise.
 */
JobItem.prototype.createState = function(optState) {
  var def = when.defer();
 ...
 * @return {when.Promise} a promise.
 */
JobItem.prototype.createTimeIndex = function() {
  var def = when.defer();
  var setKey = this.NS + ':time-index';
  this.client.zadd(setKey, this.createTime, this.id, function(err) {
    if (err) {
