/**
 * @fileoverview Fetch jobs from the to process queues,
...
FetchModel.prototype._onResponse = function(resolver, err, response) {
  ...
  jobModel.fetch().then(this._onJobItemFetch.bind(this, resolver), resolver.reject);
};

/**
 * After the jobItem has been fetched from the db update state.
 *
 * @param  {when.Resolver} resolver the promise resolver.
 * @param  {Kickq.JobItem} jobItem  the job item
 * @private
 */
FetchModel.prototype._onJobItemFetch = function(resolver, jobItem) {
  // avoid storing back to db until it is proven to be required.
  jobItem.state = states.Job.PROCESSING;
 ...
 */
FetchModel.prototype.dispose = function() {
  // cut the oxygen
  this.fetch = function(){};
