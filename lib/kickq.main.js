...
var rootLogger = logg.getLogger('');
// rootLogger.setLogLevel(logg.Level.FINEST);
rootLogger.setLogLevel(logg.Level.WARN);
...
kickq.get = function kickqGet(jobId, optCb, optSelf) {
  ...
  jobModel.fetch().then(function(job){
    ...
    try {
      ...
      console.log('kickq.get() callback raised exception: ', ex);
      def.reject(ex);
      return;
    }
